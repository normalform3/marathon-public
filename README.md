...
这是一个前后端同仓库的马拉松赛事管理系统，面向跑者、赛事主办方和管理员三类角色。项目重点展示热门赛事报名场景下的高并发受理、Redis Lua 原子去重/扣库存、RabbitMQ 异步削峰、抽签、支付状态一致性和多角色后台管理。
...
真实本地配置请放在不提交的配置文件或系统环境变量中。

## 讲解重点

- 为什么真实马拉松报名要区分“超额报名后抽签”和“限额报名报完即止”。
- 为什么使用 Caffeine + Redis + MySQL 做多级缓存，而不是只依赖数据库或 Redis。
- 为什么 Lua 只处理高并发临界区，不承载复杂业务校验。
- Redis Lua 成功但 MQ 发送失败时，如何补偿报名标记和库存。
- RabbitMQ 重复消息如何通过消费端查重和数据库唯一索引兜底。
- 为什么支付成功后才分配参赛号，避免未支付用户占用正式号码。

更详细的功能点、技术流程和维护记录见 [docs/feature-notes.md](docs/feature-notes.md)。
